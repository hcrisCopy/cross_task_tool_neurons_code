...
../cross_task_tool_neurons_data/visualizations/<model_alias>/shared_by_subset/ctd_layer_top1pct_scar_heatmap_<subset>.png
```

方法：按完整身份 `(layer, module, index)` 精确取交集，`CTD = TDN_A ∩ TDN_B ∩ TDN_C`，同时记录两两重叠和可视化。新增的 `ctd_layer_top1pct_scar_heatmap` 是 CTD 共享集合的一张逐层图，按 `layer + gate_proj/up_proj/down_proj` 分行，使用 `shared_score = min(score_A, score_B, score_C)` 表示交集神经元的保守共享强度；旧 CTD 产物存在时重跑本阶段会直接补图，并清理旧的逐层 top1% A/B/C 拆分图或聚合图。
