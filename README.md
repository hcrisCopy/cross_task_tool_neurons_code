...
../cross_task_tool_neurons_data/visualizations/<model_alias>/single_type_by_subset/layer_top10_scar_heatmap_<subset>_<A|B|C>.png
```

方法：只读 train activation。A/B/C 各自把 `tool_necessary=1` 和 `0` 分成两组，按 SCAR 分数在全模型 FFN 坐标里取 `top_k=5000`。新增的 `layer_top10_scar_heatmap` 按 `layer + gate_proj/up_proj/down_proj` 分行，展示每层每个 FFN module 内 SCAR 前 10% 神经元的分数。
...
方法：按完整身份 `(layer, module, index)` 精确取交集，`CTD = TDN_A ∩ TDN_B ∩ TDN_C`，同时记录两两重叠和可视化。
