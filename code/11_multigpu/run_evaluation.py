...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 8 data-parallel launcher for trained-model evaluation.")
...
def main() -> None:
    args = parse_args()
    from sharded_eval_common import resolve_outputs_root, run_trained_subset_dp, write_root_manifest

    subsets = selected_subsets(args.subset)
    outputs_root = resolve_outputs_root(args.outputs_dir)
    root_subsets = {}
    print(f"[Stage 8] {args.model_alias}: running subsets sequentially: {', '.join(subsets)}")
    for subset in subsets:
        summary = run_trained_subset_dp(args, script_path=STAGE8_SCRIPT, repo_root=REPO_ROOT, subset=subset)
        root_subsets[subset] = {
    ...
    print(f"[Stage 8] {args.model_alias}: finished {len(subsets)} subset(s).")
