...
STAGE4_SCRIPT = REPO_ROOT / "code" / "04_activation_extraction" / "extract_ffn_activations.py"
MODEL_PARALLEL_ALIASES = {"qwen3-32b", "llama3.3-70b"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 4 multi-GPU launcher.")
...
def base_stage4_cmd(args: argparse.Namespace, subset: str, split: str) -> list[str]:
    cmd = [
        sys.executable,
        str(STAGE4_SCRIPT),
        "--model-alias",
...
def run_checked(cmd: list[str], *, cuda_visible_devices: str | None = None) -> None:
    ...
    print("+", " ".join(cmd), f"(CUDA_VISIBLE_DEVICES={cuda_visible_devices})" if cuda_visible_devices else "")
...
def run_data_parallel(args: argparse.Namespace, gpus: list[str], subset: str, split: str) -> None:
    ...
    if num_shards > len(gpus):
        raise ValueError("--num-data-shards cannot exceed the number of GPUs for this launcher")
    if num_shards == 1:
        run_checked(base_stage4_cmd(args, subset, split), cuda_visible_devices=gpus[0])
        return
    workers: list[tuple[int, subprocess.Popen[bytes]]] = []
    for shard_index in range(num_shards):
        cmd = base_stage4_cmd(args, subset, split)
    ...
    if failed:
        raise RuntimeError(f"Stage 4 shard workers failed: {failed}")

    merge_cmd = base_stage4_cmd(args, subset, split)
...
def main() -> None:
    ...
    subsets = selected(("single_hop", "multi_hop"), args.subset)
    splits = selected(("train", "test"), args.split)
    print(f"Stage 4 multi-GPU mode: {mode}; GPUs={gpus}")

    for subset in subsets:
        for split in splits:
            if mode == "model":
                run_checked(base_stage4_cmd(args, subset, split), cuda_visible_devices=",".join(gpus))
            else:
