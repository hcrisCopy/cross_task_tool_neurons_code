...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "PreciseShield_Deepfake PSDF-5: discover A/B/C tool-decision neurons with "
            "PreciseShield FFN intermediate coordinates and Deepfake paired-shift scoring."
        )
    )
    ...
    parser.add_argument(
        "--activations-dir",
        default=None,
        help="PSDF-4 activation root; defaults to ../cross_task_tool_neurons_data/precise_shield_deepfake/activations.",
    )
    parser.add_argument(
        "--neurons-dir",
        default=None,
        help="PSDF neuron output root; defaults to ../cross_task_tool_neurons_data/precise_shield_deepfake/neurons.",
    )
    parser.add_argument(
        "--visualizations-dir",
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Deepfake-style per-layer selected ratio. k_l=ceil(top_ratio * dim_l). Default matches deepfake-code.",
    )
...
def default_root(kind: str) -> Path:
    mapping = {
        "activations": data_root() / "precise_shield_deepfake" / "activations",
    ...
    *,
    ...
    for key, pack in score_pack.items():
        ...
        if not isinstance(score, torch.Tensor) or not isinstance(mean_delta, torch.Tensor) or not isinstance(std_delta, torch.Tensor):
            raise TypeError(f"Invalid score pack for layer {key}")
        meta = meta_by_key[key]
        count = top_count(score.numel(), top_ratio, min_neurons_per_layer)
        values, indices = torch.topk(score, count)
        for rank_in_layer, (value, raw_index) in enumerate(zip(values.tolist(), indices.tolist()), start=1):
            index = int(raw_index)
            rows.append(
                {
...
def layer_dims(module_meta: list[dict[str, Any]]) -> dict[int, int]:
...
def plot_density(rows_by_type: dict[str, list[dict[str, Any]]], module_meta: list[dict[str, Any]], out_path: Path) -> None:
    dims = layer_dims(module_meta)
    layers = sorted(dims)
    if not layers:
        write_empty_plot(out_path, "PreciseShield_Deepfake TDN density")
        return
    fig, axes = plt.subplots(1, len(TASK_TYPES), figsize=(9, max(5, len(layers) * 0.22)), sharey=True)
    if len(TASK_TYPES) == 1:
        axes = [axes]
...
def plot_score_heatmap(rows: list[dict[str, Any]], out_path: Path, top_n: int, title: str) -> None:
    ...
    y_by_group = {layer: idx for idx, layer in enumerate(group_order)}
    ...
    ax.set_title(title)
    ax.set_xlabel("Global PSDF-TDN rank")
...
def plot_layer_top_score_heatmap(
    *,
    ...
    im = ax.imshow(matrix.numpy(), aspect="auto", cmap=cmap, vmin=0.0)
    ax.set_title(title)
    ax.set_xlabel(f"Selected neuron rank within top {top_ratio:g} of each layer")
    ax.set_ylabel("Layer / FFN neuron space")
...
def plot_layer_shift(layer_rows: list[dict[str, Any]], out_path: Path, title: str) -> None:
    if not layer_rows:
        write_empty_plot(out_path, title)
        return
    labels = [f"L{row['layer']}.{INTERMEDIATE_MODULE}" for row in layer_rows]
...
def expected_params(
    ...
    *,
    ...
    return {
        "stage": "psdf_05_single_type_paired_shift_neuron_discovery",
        ...
        "selection": "A/B/C separately; deterministic one-to-one label pairs inside each task type; per-layer TopRatio",
        "neuron_identity": "(layer, module=ffn_intermediate, index)",
    }


def expected_visualizations(viz_dir: Path, subset: str) -> list[Path]:
    paths = [viz_dir / f"psdf_tdn_density_heatmap_{subset}.png"]
    for task_type in TASK_TYPES:
        paths.extend(
            [
                viz_dir / f"psdf_paired_shift_heatmap_{subset}_{task_type}.png",
                viz_dir / f"psdf_layer_topratio_shift_heatmap_{subset}_{task_type}.png",
                viz_dir / f"psdf_layer_shift_{subset}_{task_type}.png",
                viz_dir / f"psdf_selected_counts_{subset}_{task_type}.png",
            ]
        )
    return paths
...
def should_skip(
    ...
    *,
    ...
    if manifest.get("params") == params:
        print(f"Skip existing PreciseShield_Deepfake single-type neurons: {out_root / subset}", flush=True)
        return True
    return False
...
def run_subset(
    ...
    *,
    ...
    if not activation_path.exists() or not meta_path.exists() or not manifest_path.exists():
        raise FileNotFoundError(f"Missing PreciseShield_Deepfake train activations for {subset}: {act_dir}")
    ...
    for task_type in TASK_TYPES:
        ...
        score_pack = compute_scores_for_layers(
            activations=activations,
            ...
            eps=args.epsilon,
            floor_ratio=args.floor_ratio,
            desc=f"{subset}/type {task_type} PSDF paired shift",
        )
        ...
        torch.save(
            {
                ...
                "scores": {
                    key: {
                        name: value.to(torch.float32) if isinstance(value, torch.Tensor) else value
        ...
        heatmap_path = viz_dir / f"psdf_paired_shift_heatmap_{subset}_{task_type}.png"
        layer_top_path = viz_dir / f"psdf_layer_topratio_shift_heatmap_{subset}_{task_type}.png"
        layer_shift_path = viz_dir / f"psdf_layer_shift_{subset}_{task_type}.png"
        selected_counts_path = viz_dir / f"psdf_selected_counts_{subset}_{task_type}.png"
        plot_score_heatmap(rows, heatmap_path, args.heatmap_top_n, f"{subset} Type {task_type}: PSDF paired shift")
        plot_layer_top_score_heatmap(
            ...
            top_ratio=args.top_ratio,
            min_neurons_per_layer=args.min_neurons_per_layer,
            title=f"{subset} Type {task_type}: top-ratio paired shift by layer",
        )
        plot_layer_shift(layer_rows, layer_shift_path, f"{subset} Type {task_type}: layer and top-neuron shifts")
        plot_selected_counts(layer_rows, selected_counts_path, f"{subset} Type {task_type}: selected neurons per layer")
        type_summary = {
            "task_type": task_type,
            "n_pairs": len(pair_rows),
        ...
        print(
            f"{subset}/type {task_type}: PSDF_TDN={len(rows)}, pairs={len(pair_rows)}, top_ratio={args.top_ratio:g}",
            flush=True,
        )

    density_path = viz_dir / f"psdf_tdn_density_heatmap_{subset}.png"
...
def main() -> None:
    ...
    print(f"PreciseShield_Deepfake compute device: {device}", flush=True)
    ...
    for subset in progress(subset_values(args.subset), desc=f"PSDF-5 {args.model_alias}", unit="subset"):
    ...
    print(f"Wrote PreciseShield_Deepfake single-type manifest: {model_root / 'manifest.json'}", flush=True)
