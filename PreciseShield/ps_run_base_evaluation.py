...
STAGE_SCRIPT = REPO_ROOT / "PreciseShield" / "ps_evaluate_base_and_delta.py"
MODEL_PARALLEL_ALIASES = {"qwen3-14b", "qwen3-32b", "llama3.3-70b"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PreciseShield PS-9 eight-GPU launcher.")
...
def resolve_outputs_root(value: str | None) -> Path:
    ...
    return (REPO_ROOT.parent / "cross_task_tool_neurons_data" / "precise_shield" / "outputs").resolve()
...
def run_subset_parallel(args: argparse.Namespace, gpus: list[str]) -> None:
    ...
    if failed:
        raise RuntimeError(f"PreciseShield base-evaluation workers failed: {failed}")


def merge_root_manifest(args: argparse.Namespace) -> None:
    root = resolve_outputs_root(args.outputs_dir) / args.model_alias / "base_evaluation"
    payload: dict[str, Any] = {"stage": "ps_09_base_evaluation", "model_alias": args.model_alias, "subsets": {}}
    for subset in selected_subsets(args.subset):
        manifest_path = root / subset / "manifest.json"
        comparison_path = resolve_outputs_root(args.outputs_dir) / args.model_alias / "trained_evaluation" / subset / "comparison_with_base_manifest.json"
        if not manifest_path.exists():
            continue
        manifest = read_json(manifest_path)
        payload["subsets"][subset] = {
            "path": str(root / subset),
            "overall": manifest.get("summary", {}),
...
def main() -> None:
    ...
    print(f"PreciseShield PS-9 mode: {mode}; GPUs={gpus}")
