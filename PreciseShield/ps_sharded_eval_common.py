...
from ps_causal_validation import add_tradeoff_deltas, intervention_rows, numeric_values, parse_interventions
from ps_common import STAGE_VERSION, dataset_manifest, ps_resolve_root, select_records, stable_sha256
...
def resolve_outputs_root(value: str | None) -> Path:
    return ps_resolve_root(value, "outputs")


def resolve_checkpoints_root(value: str | None) -> Path:
    return ps_resolve_root(value, "checkpoints")


def resolve_neurons_root(value: str | None) -> Path:
    return ps_resolve_root(value, "neurons")


def resolve_causal_root(value: str | None) -> Path:
    return ps_resolve_root(value, "causal")
...
def load_subset_test_rows(model_dataset: Path, subset: str, args: argparse.Namespace) -> list[dict[str, Any]]:
...
def prepare_shards(
    *,
    ...
    ensure_dir(work_dir)
    shard_infos = []
    for shard_index, (gpu, rows) in enumerate(zip(gpus, split_rows(data_rows, len(gpus)))):
        if not rows:
            continue
...
def read_progress_file(path: Path) -> tuple[int, int] | None:
    ...
    try:
        ...
        return None
    return int(payload.get("done", 0) or 0), int(payload.get("total", 0) or 0)
...
def run_process_group(jobs: list[dict[str, Any]], *, desc: str, repo_root: Path) -> None:
    if not jobs:
        ...
        return
    progress_total = sum(int(job.get("progress_total", 0) or 0) for job in jobs)
    processes = []
    log_files = []
    for job in jobs:
    ...
    try:
        with tqdm(total=progress_total, desc=desc, dynamic_ncols=True, unit="task") as bar:
...
def format_metric(value: Any) -> str:
    if isinstance(value, dict) and "mean" in value:
        return f"{float(value.get('mean', 0.0)):.4f}+/-{float(value.get('std', 0.0)):.4f}"
...
def write_eval_outputs(
    *,
    ...
    if n_runs == 1:
        ...
        outputs_payload: Any = sort_outputs(outputs, order)
...
def append_common_ps_eval_args(cmd: list[str], args: argparse.Namespace, subset: str) -> None:
    cmd.extend(
        [
            "--model-alias",
            args.model_alias,
...
def run_ps_trained_subset_dp(
    ...
    *,
    ...
    data_rows = load_subset_test_rows(model_dataset, subset, args)
    tool_format = infer_tool_format(args.model_alias, model_path)
    params = ps_trained_params(
        args,
        subset,
    ...
    if not args.overwrite and params_match(
        ...
        params,
        [final_dir / "outputs.json", final_dir / "per_task.jsonl", final_dir / "summary.json", final_dir / "summary_table.csv"],
    ):
        summary = read_json(final_dir / "summary.json")
        print(f"Skip existing PreciseShield trained evaluation: {final_dir}")
        print_eval_metrics("PS-8", subset, summary)
        return summary
    clean_if_exists(final_dir)

    model_manifest = dataset_manifest(model_dataset)
    ...
    for info in shard_infos:
        ...
        append_common_ps_eval_args(cmd, args, subset)
        cmd.extend(
            [
                "--dataset-dir",
    ...
    print(f"[PS-8] {subset}: {len(data_rows)} test rows -> {len(jobs)} GPU shards")
    run_process_group(jobs, desc=f"PS-8 {subset}", repo_root=repo_root)
    shard_dirs = [info["output_root"] / args.model_alias / "trained_evaluation" / subset for info in shard_infos]
    adapter_loaded = all(read_json(shard_dir / "manifest.json").get("adapter_loaded", False) for shard_dir in shard_dirs)
    summary = write_eval_outputs(
        ...
        extra_manifest={"adapter_loaded": adapter_loaded},
    )
...
def write_ps_comparison(*, args: argparse.Namespace, outputs_root: Path, subset: str) -> dict[str, Any]:
    ...
    if not base_manifest_path.exists() or not base_summary_path.exists():
        raise FileNotFoundError(f"Missing PreciseShield base evaluation outputs for {subset}: {base_dir}")
    if not trained_manifest_path.exists() or not trained_summary_path.exists():
        raise FileNotFoundError(f"Missing PreciseShield trained evaluation outputs for {subset}: {trained_dir}")
    params = {
        ...
        "trained_manifest_params": read_json(trained_manifest_path).get("params", {}),
    ...
    if not row and "path" in manifest:
        ...
        if path.exists():
            with path.open("r", encoding="utf-8", newline="") as f:
                row = find_overall_row(list(csv.DictReader(f)))
    if row:
        print(
            "[PS-9] "
...
def run_ps_base_subset_dp(
    ...
    *,
    ...
    data_rows = load_subset_test_rows(model_dataset, subset, args)
    tool_format = infer_tool_format(args.model_alias, model_path)
    params = ps_base_params(args, subset, model_path=model_path, model_dataset=model_dataset, rows=data_rows, tool_format=tool_format)
    ...
    if not args.overwrite and params_match(
        ...
        summary = read_json(final_dir / "summary.json")
        print(f"Skip existing PreciseShield base evaluation: {final_dir}")
    else:
        clean_if_exists(final_dir)
        model_manifest = dataset_manifest(model_dataset)
        work_dir = final_dir / "_shards"
        ...
        for info in shard_infos:
            ...
            jobs.append({**info, "cmd": cmd, "progress_total": len(info["rows"]) * args.n_runs})
        print(f"[PS-9] {subset}: {len(data_rows)} test rows -> {len(jobs)} GPU shards")
        run_process_group(jobs, desc=f"PS-9 {subset}", repo_root=repo_root)
        ...
        if not args.keep_shards:
            clean_if_exists(work_dir)
    print_eval_metrics("PS-9", subset, summary)
    comparison = write_ps_comparison(args=args, outputs_root=outputs_root, subset=subset)
    print_comparison_metrics(subset, comparison)
    return {"summary": summary, "comparison": comparison}


def run_ps_causal_subset_dp(
    args: argparse.Namespace,
    *,
    script_path: Path,
    repo_root: Path,
    ...
    if not args.overwrite and params_match(
        final_dir / "manifest.json",
        params,
        [final_dir / "summary_table.csv", final_dir / "cross_type_summary.csv"],
    ):
        manifest = read_json(final_dir / "manifest.json")
        print(f"Skip existing PreciseShield causal validation: {final_dir}")
    ...
    for info in shard_infos:
        ...
        if args.model_path:
            cmd.extend(["--model-path", args.model_path])
        jobs.append({**info, "cmd": cmd, "progress_total": len(info["rows"]) * len(interventions)})
    print(f"[PS-10] {subset}: {len(data_rows)} test rows -> {len(jobs)} GPU shards")
    run_process_group(jobs, desc=f"PS-10 {subset}", repo_root=repo_root)
    shard_dirs = [info["output_root"] / args.model_alias / subset for info in shard_infos]
    manifest = write_ps_causal_outputs(
        out_dir=final_dir,
        shard_dirs=shard_dirs,
        data_rows=data_rows,
        model_alias=args.model_alias,
...
def print_causal_metrics(subset: str, rows: list[dict[str, Any]]) -> None:
    if not rows:
        print(f"[PS-10] {subset}: no causal summary rows")
        return
    ctd = next((row for row in rows if row.get("intervention") == "Mask-PS-CTD"), rows[0])
    print(
        "[PS-10] "
