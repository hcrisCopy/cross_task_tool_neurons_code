...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PreciseShield PS-8 data-parallel launcher.")
...
def main() -> None:
    args = parse_args()
    from ps_sharded_eval_common import eval_overall, resolve_outputs_root, run_ps_trained_subset_dp, selected_subsets, write_root_manifest

    subsets = selected_subsets(args.subset)
    outputs_root = resolve_outputs_root(args.outputs_dir)
    root_subsets = {}
    print(f"[PS-8] {args.model_alias}: running subsets sequentially: {', '.join(subsets)}")
    for subset in subsets:
        summary = run_ps_trained_subset_dp(args, script_path=STAGE_SCRIPT, repo_root=REPO_ROOT, subset=subset)
        root_subsets[subset] = {
            "path": str(outputs_root / args.model_alias / "trained_evaluation" / subset),
            "overall": eval_overall(summary),
        }
    write_root_manifest(
        outputs_root / args.model_alias / "trained_evaluation" / "manifest.json",
        stage="ps_08_trained_evaluation",
        model_alias=args.model_alias,
        subsets=root_subsets,
    )
    print(f"[PS-8] {args.model_alias}: finished {len(subsets)} subset(s).")
