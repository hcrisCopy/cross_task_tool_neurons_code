...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PreciseShield PS-8 eight-GPU launcher.")
...
def run_subset_parallel(args: argparse.Namespace, gpus: list[str]) -> None:
    ...
    if failed:
        raise RuntimeError(f"PreciseShield trained-evaluation workers failed: {failed}")


def merge_root_manifest(args: argparse.Namespace) -> None:
    root = resolve_outputs_root(args.outputs_dir) / args.model_alias / "trained_evaluation"
    payload: dict[str, Any] = {"stage": "ps_08_trained_evaluation", "model_alias": args.model_alias, "subsets": {}}
...
def main() -> None:
    ...
    print(f"PreciseShield PS-8 mode: {mode}; GPUs={gpus}")
