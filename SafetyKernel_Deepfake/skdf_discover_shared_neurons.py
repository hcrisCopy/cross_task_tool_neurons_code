...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SafetyKernel_Deepfake SKD-6: discover shared SKD-CTD neurons by A/B/C intersection.")
    parser.add_argument("--model-alias", required=True)
    parser.add_argument(
        "--neurons-dir",
        default=None,
        help="SKD neuron root; defaults to ../cross_task_tool_neurons_data/safety_kernel_deepfake/neurons.",
    )
    parser.add_argument(
        "--visualizations-dir",
        default=None,
...
def read_tdn(root: Path, subset: str, task_type: str) -> dict[tuple[int, str, int], dict[str, Any]]:
    ...
    if not rows:
        raise ValueError(f"Empty SafetyKernel_Deepfake TDN file: {path}")
...
def load_module_dims(single_root: Path, subset: str) -> dict[tuple[int, str], int]:
    ...
    if not meta_path.exists():
        return {}
    return {(int(meta["layer"]), str(meta["module"])): int(meta["dim"]) for meta in read_json(meta_path)}
...
def module_groups(module_dims: dict[tuple[int, str], int], rows: list[dict[str, Any]]) -> list[tuple[int, str]]:
    groups = list(module_dims) if module_dims else [(int(row["layer"]), str(row["module"])) for row in rows]
...
def plot_density(rows: list[dict[str, Any]], module_dims: dict[tuple[int, str], int], out_path: Path) -> None:
    ...
    if not layers:
        write_empty_plot(out_path, "SKD-CTD Shared Neurons")
        return
    counts = Counter((int(row["layer"]), str(row["module"])) for row in rows)
    matrix = []
    for layer in layers:
        matrix.append([counts.get((layer, module), 0) / max(module_dims.get((layer, module), 1), 1) for module in MODULE_ORDER])
    fig, ax = plt.subplots(figsize=(4.8, max(5, len(layers) * 0.22)))
    im = ax.imshow(matrix, aspect="auto", cmap="magma")
    ax.set_title("SKD-CTD Shared Neurons")
...
def plot_score_heatmap(rows: list[dict[str, Any]], out_path: Path, score_field: str, top_n: int, title: str) -> None:
    ...
    for x, row in enumerate(selected):
        matrix[y_by_group[(int(row["layer"]), str(row["module"]))]][x] = float(row[score_field])
...
def expected_params(
    ...
    *,
    ...
    return {
        "stage": "skdf_06_shared_paired_shift_neuron_discovery",
        ...
        "selection": "SKD_CTD = SKD_TDN_A intersection SKD_TDN_B intersection SKD_TDN_C",
...
def expected_visualizations(viz_dir: Path, subset: str) -> list[Path]:
    return [
        viz_dir / f"skd_ctd_density_heatmap_{subset}.png",
        viz_dir / f"skd_ctd_shift_min_heatmap_{subset}.png",
        viz_dir / f"skd_ctd_shift_mean_heatmap_{subset}.png",
        viz_dir / f"skd_ctd_layer_top1pct_shift_heatmap_{subset}.png",
    ]
...
def should_skip(
    ...
    *,
    ...
    if manifest.get("params") == params:
        print(f"Skip existing SafetyKernel_Deepfake shared neurons: {out_dir}", flush=True)
        return True
    return False
...
def run_subset(
    ...
    *,
    ...
    if not single_manifest_path.exists():
        raise FileNotFoundError(f"Missing SKD single-type manifest: {single_manifest_path}")
        ...
        share_rows.append(
            {
                ...
                "subset": subset,
                "task_type": task_type,
                "skd_tdn_count": len(sets[task_type]),
                "skd_ctd_count": len(ctd),
    ...
    density_path = viz_dir / f"skd_ctd_density_heatmap_{subset}.png"
    score_min_path = viz_dir / f"skd_ctd_shift_min_heatmap_{subset}.png"
    score_mean_path = viz_dir / f"skd_ctd_shift_mean_heatmap_{subset}.png"
    layer_top_path = viz_dir / f"skd_ctd_layer_top1pct_shift_heatmap_{subset}.png"
    plot_density(ctd_rows, module_dims, density_path)
    plot_score_heatmap(ctd_rows, score_min_path, "score_min", args.heatmap_top_n, f"{subset} SKD-CTD shared paired shift (min)")
    plot_score_heatmap(ctd_rows, score_mean_path, "score_mean", args.heatmap_top_n, f"{subset} SKD-CTD shared paired shift (mean)")
    plot_layer_top_shared_score_heatmap(
        ...
        title=f"{subset} SKD-CTD: top 1% shared paired shift by layer/module",
    )
    ...
    summary = {
        ...
        "neuron_set": "SKD_CTD",
        "skd_tdn_counts": {task_type: len(sets[task_type]) for task_type in TASK_TYPES},
        "skd_ctd_count": len(ctd),
...
def main() -> None:
    ...
    for subset in progress(subset_values(args.subset), desc=f"SKD-6 {args.model_alias}", unit="subset"):
        ...
        summary_rows.append(
            {
                ...
                "method": METHOD_NAME,
                "neuron_set": "SKD_CTD",
                "selected_neurons": summary["skd_ctd_count"],
            }
        )
    ...
    write_csv_rows(
        ...
        global_share_rows,
        fieldnames=["model_alias", "subset", "task_type", "skd_tdn_count", "skd_ctd_count", "share_rate"],
    )
    write_json(shared_dir / "manifest.json", root_manifest)
    print(f"Wrote SafetyKernel_Deepfake shared manifest: {shared_dir / 'manifest.json'}", flush=True)
